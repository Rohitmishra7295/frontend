     ...
     <Link  to='/'> <img src={assets.logo} className='w-36' alt="Logo" /> </Link>
      ...
      <div className='flex items-center gap-6'>
              ...
              <Link to='/cart' className='relative' >
                   ...
                   <p className='absolute right-[-5px] bottom-[-5px] w-4 text-center leading-4 bg-black text-white aspect-square  round-full text-[8px]'>{getCartCount()}</p>
              </Link>
              <img onClick={() => setVisible(true)} src={assets.menu_icon} className='w-5 cursor-pointer sm:hidden' alt="" />
         </div>
         {/* side bar menu for small screen */}
         <div className={`absolute top-0 right-0 bottom overflow-hidden bg-white transition-all ${visible ? 'w-full' :'w-0'  }`}>
...
//               key={item}
//               onClick={() => {
//                 setVisible(false); // Close the menu on link click
